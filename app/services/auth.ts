import { TResponse, TSignUp, TSignIn, TSignInResponse } from "@/app/type";
...
export const fetchSignIn = async (
  data: TSignIn,
): Promise<TResponse<TSignInResponse>> => {
  try {
    const response = await fetch(
    ...
    const result: TResponse<TSignInResponse> = await response.json();
